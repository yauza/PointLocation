...
def simpleCase1(trNode: TrapezoidNode, edge, dag, visualizer = None):
    old_tr = trNode
...
def hardCase1(dag, intersectingTrapezoids, segment, visualizer = None):
...
def algo(edges, visualizer = None):
    ...
    for edge in edges:
        ...
        findIntersectingTrapezoids1(dag.root, edge, intersectingTrapezoids)
        if len(intersectingTrapezoids) == 1:
            simpleCase1(intersectingTrapezoids[0], edge, dag, visualizer)
        else:
            hardCase1(dag, intersectingTrapezoids, edge, visualizer)
